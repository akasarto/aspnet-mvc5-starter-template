...
namespace App.UI.Mvc5.Areas.Users.Models
{
	public class AccountSignUpViewModelValidator : AbstractValidator<AccountSignUpViewModel>
	{
		...
		public AccountSignUpViewModelValidator(IIdentityRepository identityRepository)
		{
			...
			RuleFor(model => model.Name).Length(0, _Constants.UsersFullNameMaxLength).WithMessage(GlobalizationManager.GetLocalizedString<AreaResources>("_Validation_MaxLength"));
			...
			RuleFor(model => model.Email).Must(BeUniqueEmail).WithMessage(GlobalizationManager.GetLocalizedString<AreaResources>("_Validation_EmailTakenError"));
			...
			RuleFor(model => model.Password2).Equal(model => model.Password).WithMessage(GlobalizationManager.GetLocalizedString<AreaResources>("_Validation_PasswordsDoNotMatchError"));

			// Terms Of Use Acceptance
			RuleFor(model => model.TermsAcceptance).SetValidator(new RequiredCheckboxValidator()).WithMessage(GlobalizationManager.GetLocalizedString<AreaResources>("_Validation_TermsAcceptanceError"));
		}
